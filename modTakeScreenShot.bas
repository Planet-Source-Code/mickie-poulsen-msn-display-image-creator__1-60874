...
Public Sub PartialScreenShot(DestinationDC As Long, SavePic As PictureBox, Top As Long, Left As Long, Width As Long, Height As Long)
Dim ScreenDC As Long, retval As Long, DeskWND As Long
    DeskWND = GetDesktopWindow
    ScreenDC = GetDC(DeskWND)
    SavePic.AutoRedraw = True
    SavePic.Height = Height
    SavePic.Width = Width
    retval = BitBlt(DestinationDC, 0, 0, Width, Height, ScreenDC, Top, Left, SRCCOPY)
    retval = BitBlt(SavePic.hdc, 0, 0, Width, Height, ScreenDC, Top, Left, SRCCOPY)
    Call ReleaseDC(DeskWND, ScreenDC)
End Sub
